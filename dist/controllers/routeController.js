...
class RouteController {
    constructor() {
        ...
        this.routeEmitter = new events_1.EventEmitter();
    }
    ...
    calculateBestRoute(startStation, endStation) {
    ...
    findDirectRoute(startStation, endStation) {
        ...
        const relevantStops = bestRoute.stops.slice(Math.min(startIndex, endIndex), Math.max(startIndex, endIndex) + 1);
        return {
            type: 'direct',
    ...
    findRouteWithTransfer(startStation, endStation) {
        const startRoutes = this.testRoutes.filter(route => route.stops.some(stop => stop.name === startStation));
        const endRoutes = this.testRoutes.filter(route => route.stops.some(stop => stop.name === endStation));
        ...
        for (const transferStation of this.mainTransferStations) {
            ...
            const routesToTransfer = this.testRoutes.filter(route => route.stops.some(stop => stop.name === startStation) &&
                route.stops.some(stop => stop.name === transferStation));
            // Buscar rutas que conecten transbordo con destino
            const routesFromTransfer = this.testRoutes.filter(route => route.stops.some(stop => stop.name === transferStation) &&
    ...
    findShortestPath(startStation, endStation) {
        ...
        this.testRoutes.forEach(route => {
            ...
            for (let i = 0; i < route.stops.length - 1; i++) {
                ...
                if (!graph.has(b))
                    graph.set(b, new Map());
                graph.get(a).set(b, Math.min((_a = graph.get(a).get(b)) !== null && _a !== void 0 ? _a : Infinity, weight));
                graph.get(b).set(a, Math.min((_b = graph.get(b).get(a)) !== null && _b !== void 0 ? _b : Infinity, weight));
            }
        });
        ...
        while (visited.size < graph.size) {
            ...
            if (current === null || current === endStation)
                break;
            visited.add(current);
            ...
            for (const [neighbor, weight] of neighbors) {
                ...
                if (alt < ((_a = distances.get(neighbor)) !== null && _a !== void 0 ? _a : Infinity)) {
    ...
    findShortestPathAStar(startStation, endStation) {
        ...
        const openSet = new Set([startStation]);
        ...
        this.testRoutes.forEach(route => {
            for (let i = 0; i < route.stops.length - 1; i++) {
                ...
                if (!graph.has(current))
                    graph.set(current, new Map());
                if (!graph.has(next))
                    graph.set(next, new Map());
        ...
        while (openSet.size > 0 && iterationCount < maxIterations) {
            ...
            let current = Array.from(openSet).reduce((a, b) => (fScore.get(a) || Infinity) < (fScore.get(b) || Infinity) ? a : b);
    ...
    findGreedyRoute(startStation, endStation) {
        const startTime = performance.now();
    ...
    getRoutesForPath(path) {
        const routes = [];
        const visitedStations = new Set();
        for (const station of path) {
            const route = this.testRoutes.find(route => route.stops.some(stop => stop.name === station) &&
